 ...
 * {@link ModelProperty}.
 * <P>
 * Persisting via this {@link ModelPersistor} will be forwarded to the
...
public abstract class ModelPersistor<ModelType, TargetPropertyType> {
	...
	/**
	 * Returns the {@link ModelProperty} this {@link ModelPersistor} is
	 * able to persist.
	 * 
	 * @return The property this {@link ModelPersistor} has registered
	 *         itself on its parent {@link ModelContainer} to persist; never null
	 */
	 ...
	 * {@link ModelPersistor}'s property is indexed, the used index context
	 * has an impact on the execution's result.
	 * <P>
	 * For determining the instances to persist, an empty index context is used.
	 * <P>
	 * Since the used index context can point to 0-n instances for this
	 * {@link ModelPersistor}'s property, there are also 0-n persisting
	 * executions to be expected, which is the reason why this method returns a list
	 * of results, not one.
	 * 
	 ...
	 * <P>
	 * Will be called for every instance of the type this
	 * {@link ModelPersistor}'s property points to when its parent
	 * {@link ModelContainer} persists its model.
	 * <P>
	 * Note that a {@link ModelPersistor} is excepted to persist the
	 * instance its property points to <b>and all of its sub instances of possible
	 * sub properties<b>. If it does not do that and such a sub instance has pending
	 ...
	 *         persisting). Will be used to update this
	 *         {@link ModelPersistor}'s parent {@link ModelContainer}'s
	 *         model.
	 * @throws Throwable
